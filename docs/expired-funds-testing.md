...
Before deployment, check that the selected environment's pipeline variable groups provide the secret `EmployerFinanceJobsApimSubscriptionKey`. The AT and DEMO stages define `EmployerFinanceJobsApimUrl` explicitly using their verified gateway URLs; other environments must supply their URL through pipeline variables. The shared `das-employer-config` schema added these required Outer API settings on 21 September 2026, with no defaults. The deployment validates both inputs before deploying infrastructure, rejects missing values and unresolved macros such as `$(EmployerFinanceJobsApimUrl)`, and passes both values to Generate Configuration. A missing value can prevent deployment even though the expiry workflow itself calls the Finance API directly. After changing YAML or variable-group values, queue a new run of this branch so it uses the updated configuration.
